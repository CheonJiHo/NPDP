...
- 데이터의 입출력에 대한 데이터 플로우를 확실히 파악하여 불필요한 메서드 사용을 지양하고 프로젝트 완성 시간을 단축하며 검토에 대한 시간을 확보해야겠다고 생각했습니다.
- API 호출이 잦아지면서 어디에서 무엇을 불러오는지 헷갈릴 때가 있었습니다. API를 모듈화하고 따로 관리하는것이 좋을 것 같다고 생각합니다.
- 각 페이지에서 공통으로 쓰이는 함수에만 vuex를 사용하여 최적화 하기위해 프로젝트 설계단계에서 API명세서에 공통항목을 정확히 파악해야 한다고 느꼈습니다.
